...
import { getAllMedicalRecords } from "/src/api/api.js";
...
export default function AllMedicalRecords({ doctorId }) {
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      try {
        const data = await getAllMedicalRecords(doctorId);
        setRecords(data.records || []);
      } catch (error) {
        toast.error(error.message ?? "Gagal memuat semua rekam medis");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy">Semua Rekam Medis</h1>
          <p className="text-slate-500 text-sm">Rekam medis dari seluruh pasien di klinik</p>
        </div>
