      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="border-l-4 border-l-teal">
          <CardHeader>
            ...
            <CardTitle className="text-3xl font-bold text-teal mt-2">{totalPatients}</CardTitle>
          </CardHeader>
        </Card>
        <Card className="border-l-4 border-l-teal">
          <CardHeader>
            <CardDescription className="text-navy/70 font-medium">Antrean Aktif</CardDescription>
            <CardTitle className="text-3xl font-bold text-teal mt-2">{activeQueue}</CardTitle>
          </CardHeader>
        </Card>
        <Card className="border-l-4 border-l-teal">
          <CardHeader>
            <CardDescription className="text-navy/70 font-medium">Pemasukan Hari Ini</CardDescription>
            <CardTitle className="text-3xl font-bold text-teal mt-2">{formatCurrency(incomeToday)}</CardTitle>
          </CardHeader>
        </Card>
      </div>
      <Card>
        <CardHeader>
          <CardTitle className="text-lg font-bold">Antrean Terkini</CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          {queues.slice(0, 5).map((queue) => (
            <div key={queue.id} className="flex items-center justify-between rounded-lg bg-sky-blue/50 border border-navy/10 px-4 py-3 hover:bg-sky-blue/70 hover:shadow-md transition-all">
              <div>
